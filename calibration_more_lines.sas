...
%let dir=D:\Julia\Studia\Magisterka SGH\I semestr\Credit Scoring\PROCSS_SIMULATION\;
...
pd_ins=1/(1+exp(-(-0.033338046*risk_ins_score+10.590644636)));
...
class year;
var pd_ins p default12;
output out=test3__ mean()=;
format pd_ins p default12 percent12.4;
where product='ins';
run;

proc rank data=test2 groups=10 out=rangi;
var risk_ins_score;
ranks ranga;
where product='ins';
run;
...
pd_css=1/(1+exp(-(-0.028157974*risk_css_score+8.0230458133)));
drop psc: SCORECARD_POINTS;
run;
...
data test2;
set test;
pd_cross_css=1/(1+exp(-(-0.021018686*risk_cross_css_score+9.8271882625)));
run;
...
pr=1/(1+exp(-(0.01943967*response_score-5.25827243)));
drop psc: SCORECARD_POINTS;
run;
...
proc gplot data=cash_cum;
plot profit_cum*pd_css;
plot profit_cum*ar;
where pd_css<0.4;
run;
quit;
...
run;
proc rank data=instalment out=instalment_rank groups=6;
var pr pd_ins;
ranks rpr rpd_ins;
run;
...
run;
%let pd_ins1=0.0255;
%let pd_ins2=0.006; %let pr2=0.28;
/*instalment loan then cash*/
